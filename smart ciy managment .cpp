...
// grows the citizen array whenever it's full
void expandCitizenCapacity() {
    ...
    cout << "\n Citizen capacity expanded to " << newCapacity << endl;
}

// shrinks it back down once too much space is wasted
void shrinkCitizenCapacity() {
    ...
    cout << "\n Citizen capacity shrunk to " << newCapacity << endl;
}
...
void expandFIRCapacity() {
    ...
    cout << "\n FIR capacity expanded to " << newCapacity << endl;
}
...
void shrinkFIRCapacity() {
    ...
    cout << "\n FIR capacity shrunk to " << newCapacity << endl;
}
...
void expandPropertyCapacity() {
    ...
    cout << "\n Property capacity expanded to " << newCapacity << endl;
}
...
void shrinkPropertyCapacity() {
    ...
    cout << "\n Property capacity shrunk to " << newCapacity << endl;
}
...
void expandMedicalCapacity() {
    ...
    cout << "\n Medical capacity expanded to " << newCapacity << endl;
}
...
void shrinkMedicalCapacity() {
    ...
    cout << "\n Medical capacity shrunk to " << newCapacity << endl;
}
...
void expandVotingCapacity() {
    ...
    cout << "\n Voting capacity expanded to " << newCapacity << endl;
}

void shrinkVotingCapacity() {
    if (votingCapacity <= INITIAL_SIZE) return;
    if (votingCount > votingCapacity / 2) return;

    int newCapacity = votingCapacity / GROWTH_FACTOR;
    if (newCapacity < INITIAL_SIZE) newCapacity = INITIAL_SIZE;
    ...
    cout << "\n Voting capacity shrunk to " << newCapacity << endl;
}
...
// checks a CNIC against citizens, properties and medical records
bool isDuplicateCNIC(const char* cnic, int currentIndex = -1) {
    ...
    for (int i = 0; i < propertyCount; i++) {
        if (propertyRecords[i] != nullptr) {
            char storedCNIC[100];
            strcpy(storedCNIC, propertyRecords[i] + 200);
...
// citizen record = name, cnic, address, phone packed into one 500 byte block
...
void addCitizen() {
    ...
    strcpy(citizenRecords[citizenCount] + 400, phone);
...
void updateCitizen() {
    ...
    for (int i = 0; i < citizenCount; i++) {
        if (citizenRecords[i] != nullptr && strcmp(citizenRecords[i] + 100, searchCNIC) == 0) {
            ...
            strcpy(citizenRecords[i] + 400, phone);
...
// property record = propID, owner, cnic, type, address, status in a 700 byte block
...
void addProperty() {
    ...
    propertyRecords[propertyCount] = new char[700];
    ...
    strcpy(propertyRecords[propertyCount] + 600, status);
...
// frees everything before the program exits
void cleanupMemory() {
    if (citizenRecords != nullptr) {
        for (int i = 0; i < citizenCount; i++) {
            delete[] citizenRecords[i];
